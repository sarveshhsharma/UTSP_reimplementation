...
parser.add_argument('--num_of_nodes', type=int, default=100, help='Graph Size') #200
...
tsp_instances = np.load('UTSP_Mine/UTSP/data/train_tsp_instance_%d.npy'%args.num_of_nodes)
...
tsp_sols = np.load('UTSP_Mine/UTSP/data/train_tsp_sol_%d.npy'%args.num_of_nodes)
...
def train(epoch):
    ...
    for batch in train_loader:
        ...
        adj *= mask 
        output = []
        for i in range(f0.size(0)):      
            output_i = (model(f0[i],adj[i]))
            output.append(output_i)
            output_tensor = torch.stack(output) 

        TSPLoss_constaint,Heat_mat = TSPLoss(SctOutput=output_tensor,distance_matrix=distance_m,num_of_nodes=args.num_of_nodes)
        Heat_mat_diagonals = [torch.diagonal(mat) for mat in Heat_mat]
        Heat_mat_diagonals = torch.stack(Heat_mat_diagonals, dim=0)
        Nrmlzd_constraint = (1. - torch.sum(output_tensor,2))**2
...
for i in range(1,args.EPOCHS+1):
    train(i)
    #if (i>=200)and(i%10 == 0):
    torch.save(model.state_dict(),'UTSP_Mine/UTSP/Saved_Models/TSP_%d/scatgnn_layer_%d_hid_%d_model_%d_temp_%.3f.pth'%(args.num_of_nodes,args.nlayers,args.hidden,i,args.temperature))
